...
À gauche figurent les N entrées du neurones.

En fonction des valeurs observées sur les entrées, le neurone calcule alors la somme des entrées, pondérées par leur poids respectif. L'expression en est la suivante : 
...
Dans le cas de la fonction ReLU, le fonctionnement du neurone est donc le suivant :
    ...
    Le neurone calcule la somme pondérée des entrées S.
    Le passage de cette somme S dans la fonction d'activation donne la sortie y du neurone :
...
Un réseau de neurones consiste simplement à connecter des neurones entre eux. La sortie d'un neurone constitue une entrée pour un ou plusieurs autres neurones.
...
Lorsque les neurones ont une entrée connectée à la sortie de chaque neurone de la couche précédente, on dit que « le réseau est dense ». C'est le modèle le plus courant que vous rencontrerez.

Reprenons l'enchaînement des opérations effectuées par le réseau quand on lui présente un exemple de la base de données (en inférence) :

    Le vecteur de caractéristiques de cet exemple est placé sur la couche d'entrée.
    Chaque neurone de la couche 1 calcule ses sorties en fonction de ses poids et de ces entrées.
    Ces sorties sont communiquées à la couche 2, qui procède de la même façon.
    Les calculs se poursuivent ainsi jusqu'à la couche de sortie.
...
Comme annoncé auparavant tous ces poids sont initialement fixés aléatoirement. Le réseau prend donc ses décisions initialement au hasard. Il va donc falloir que le réseau règle ses poids, ce que nous allons voir maintenant.
